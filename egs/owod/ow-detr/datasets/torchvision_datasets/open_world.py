...
VOC_COCO_CLASS_NAMES_NEW = tuple(itertools.chain(VOC_CLASS_NAMES, T2_CLASS_NAMES, T3_CLASS_NAMES, T4_CLASS_NAMES, UNK_CLASS))
VOC_COCO_CLASS_NAMES_OLD = tuple(itertools.chain(OLD_VOC_CLASS_NAMES, OLD_T2_CLASS_NAMES, OLD_T3_CLASS_NAMES, OLD_T4_CLASS_NAMES, UNK_CLASS))
...
class OWDetection(VisionDataset):
    ...
    def __init__(self,
        ...
        if args.split == 'owod':
            self.VOC_COCO_CLASS_NAMES = VOC_COCO_CLASS_NAMES_OLD
        elif args.split == 'owdetr':
            self.VOC_COCO_CLASS_NAMES = VOC_COCO_CLASS_NAMES_NEW
        self.CLASS_NAMES = self.VOC_COCO_CLASS_NAMES
    ...
    def load_instances(self, img_id):
        ...
        for obj in target['annotation']['object']:
            ...
            bbox[1] -= 1.0
            instance = dict(
                category_id=self.VOC_COCO_CLASS_NAMES.index(cls),
                bbox=bbox,
                area=(bbox[2] - bbox[0]) * (bbox[3] - bbox[1]),
                image_id=img_id
            )
            instances.append(instance)
        # selective_region = target['annotation']['selective_region']['bndbox']
        return target, instances #, selective_region
    ...
    def __getitem__(self, index):
        ...
        if 'selective_region' in annotation:
            if target['annotation']['selective_region']['bndbox'] is not None:
                selective_region = target['annotation']['selective_region']['bndbox']
                if isinstance(selective_region, dict):
                    regions.append([float(selective_region[x]) for x in ["xmin", "ymin", "xmax", "ymax"]])
                else:
                    for region in selective_region:
        ...
        if len(regions) == 0:
            ...
            target = dict(
                ...
                area=torch.tensor([i['area'] for i in instances], dtype=torch.float32),
                boxes=torch.as_tensor([i['bbox'] for i in instances], dtype=torch.float32),
                selective_region=torch.as_tensor(regions, dtype=torch.float32),
                orig_size=torch.as_tensor([int(h), int(w)]),
                size=torch.as_tensor([int(h), int(w)]),
                iscrowd=torch.zeros(len(instances), dtype=torch.uint8)
            )
