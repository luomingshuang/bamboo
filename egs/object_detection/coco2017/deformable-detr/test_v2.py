...
# plot box by opencv
def plot_result(pil_img, prob, boxes, save_results_dir, save_name=None, imshow=False, imwrite=False):
    ...
    if imwrite:
        if not os.path.exists("{}".format(save_results_dir)):
            os.makedirs("{}".format(save_results_dir))
        cv2.imwrite('{}/{}'.format(save_results_dir, save_name), opencvImage)
...
def main(args):
    ...
    for image_item in tqdm(image_file_path):
        ...
        ori_size = image.size
        scores, boxes, waste_time = detect(image, model, transform)
        plot_result(image, scores, boxes, args.test_results_dir, save_name=image_item, imshow=False, imwrite=True)
    ...
    for _ in range(test_interval):
        with torch.no_grad():
            ...
            # print(outputs['pred_logits'].softmax(-1)[0, :, :-1])
            probas = outputs['pred_logits'].softmax(-1)[0, :, :-1]
            keep = probas.max(-1).values > 0.7 # prob_threshold, please keep same as detect function
